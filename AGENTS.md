...
- Each app should have its own local `.env.local`, checked-in `.env.example`, and `Dockerfile`.
- Vite app base paths should come from `VITE_BASE_PATH` through app-local `.env.local` files.
...
- Frontend code should call relative `/backend/*` URLs. In development, configure Vite `server.proxy['/backend']` using `VITE_API_PROXY_TARGET`; rewrite `/backend` to `/` only when the proxy target host is `localhost` or `127.0.0.1`.
