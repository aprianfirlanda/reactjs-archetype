...
- Frontend code should call relative `/backend/*` URLs. In development, configure Vite `server.proxy['/backend']` using `VITE_API_PROXY_TARGET` and rewrite `/backend` to `/` before forwarding.
